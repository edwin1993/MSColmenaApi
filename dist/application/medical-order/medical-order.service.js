"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.MedicalOrderService = void 0;
const common_1 = require("@nestjs/common");
const medical_order_entity_1 = require("../../domain/medical-order/medical-order.entity");
let MedicalOrderService = class MedicalOrderService {
    medicalOrderRepository;
    constructor(medicalOrderRepository) {
        this.medicalOrderRepository = medicalOrderRepository;
    }
    async create(medicalOrder) {
        return this.medicalOrderRepository.create(new medical_order_entity_1.MedicalOrder(null, medicalOrder.appointmentId, medicalOrder.description, medicalOrder.expirationDate, medicalOrder.specialty, null, null));
    }
    async findById(medicalOrderId) {
        return this.medicalOrderRepository.findById(medicalOrderId);
    }
    async findByAppointmentId(appointmentId) {
        return this.medicalOrderRepository.findByAppointmentId(appointmentId);
    }
    async findAll() {
        return this.medicalOrderRepository.findAll();
    }
    async update(medicalOrderId, medicalOrder) {
        return this.medicalOrderRepository.update(medicalOrderId, medicalOrder);
    }
    async delete(medicalOrderId) {
        return this.medicalOrderRepository.delete(medicalOrderId);
    }
    async addMedication(medicalOrderId, medicationId) {
        const medicalOrder = await this.medicalOrderRepository.findById(medicalOrderId);
        if (!medicalOrder) {
            throw new common_1.BadRequestException('Orden médica no encontrada');
        }
        return this.medicalOrderRepository.addMedication(medicalOrderId, medicationId);
    }
    async removeMedication(medicalOrderId, medicationId) {
        return this.medicalOrderRepository.removeMedication(medicalOrderId, medicationId);
    }
    async getMedications(medicalOrderId) {
        return this.medicalOrderRepository.getMedications(medicalOrderId);
    }
};
exports.MedicalOrderService = MedicalOrderService;
exports.MedicalOrderService = MedicalOrderService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, common_1.Inject)('MedicalOrderRepository')),
    __metadata("design:paramtypes", [Object])
], MedicalOrderService);
//# sourceMappingURL=medical-order.service.js.map